export const translations = {
  // --- LAYOUT ---
  layout: {
    en: { 
      selectLang: "Select Language", 
      saveDefault: "Continue", 
      name: "English",
      headOffice: "Head Office",
      kisanView: "Kisan View",
      gadiMaalik: "Gaadi Maalik",
      driverBhai: "Driver Bhai",
      dashboard: "Dashboard",
      fleet: "Fleet Management",
      analytics: "Telemetry Logs",
      routes: "Route Analysis",
      alerts: "Critical Alerts",
      settings: "System Settings"
    },
    hi: { 
      selectLang: "भाषा चुनें", 
      saveDefault: "जारी रखें", 
      name: "हिंदी",
      headOffice: "मुख्य कार्यालय",
      kisanView: "किसान दृश्य",
      gadiMaalik: "गाड़ी मालिक",
      driverBhai: "ड्राइवर भाई",
      dashboard: "डैशबोर्ड",
      fleet: "वाहन प्रबंधन",
      analytics: "टेलीमेट्री लॉग",
      routes: "मार्ग विश्लेषण",
      alerts: "गंभीर अलर्ट",
      settings: "सिस्टम सेटिंग्स"
    },
    gu: { 
      selectLang: "ભાષા પસંદ કરો", 
      saveDefault: "ચાલુ રાખો", 
      name: "ગુજરાતી",
      headOffice: "હેડ ઓફિસ",
      kisanView: "કિસાન દૃશ્ય",
      gadiMaalik: "ગાડી માલિક",
      driverBhai: "ડ્રાઈવર ભાઈ",
      dashboard: "ડેશબોર્ડ",
      fleet: "વાહન વ્યવસ્થાપન",
      analytics: "ટેલિમેટ્રી લૉગ",
      routes: "રૂટ વિશ્લેષણ",
      alerts: "ગંભીર અલર્ટ",
      settings: "સિસ્ટમ સેટિંગ્સ"
    },
    gj: { 
      selectLang: "ભાષા પસંદ કરો", 
      saveDefault: "ચાલુ રાખો", 
      name: "ગુજરાતી",
      headOffice: "હેડ ઓફિસ",
      kisanView: "કિસાન દૃશ્ય",
      gadiMaalik: "ગાડી માલિક",
      driverBhai: "ડ્રાઈવર ભાઈ",
      dashboard: "ડેશબોર્ડ",
      fleet: "વાહન વ્યવસ્થાપન",
      analytics: "ટેલિમેટ્રી લૉગ",
      routes: "રૂટ વિશ્લેષણ",
      alerts: "ગંભીર અલર્ટ",
      settings: "સિસ્ટમ સેટિંગ્સ"
    },
    pa: { 
      selectLang: "ਭਾਸ਼ਾ ਚੁਣੋ", 
      saveDefault: "ਜਾਰੀ ਰੱਖੋ", 
      name: "ਪੰਜਾਬੀ",
      headOffice: "ਹੈੱਡ ਅਫੀਸ",
      kisanView: "ਕਿਸਾਨ ਦ੍ਰਿਸ਼",
      gadiMaalik: "ਗਾੜੀ ਮਾਲਕ",
      driverBhai: "ਡ੍ਰਾਈਵਰ ਭਾਈ",
      dashboard: "ਡੈਸ਼ਬੋਰਡ",
      fleet: "ਫਲੀਟ ਪ੍ਰਬੰਧਨ",
      analytics: "ਟੈਲੀਮੈਟਰੀ ਲੌਗ",
      routes: "ਰੂਟ ਵਿਸ਼ਲੇਸ਼ਣ",
      alerts: "ਨਾਜ਼ੁਕ ਚੇتਾਵਨੀਆਂ",
      settings: "ਸਿਸਟਮ ਸੈਟਿੰਗਾਂ"
    },
    mr: { 
      selectLang: "भाषा निवडा", 
      saveDefault: "सुरू ठेवा", 
      name: "मराठी",
      headOffice: "मुख्य कार्यालय",
      kisanView: "किसान दृश्य",
      gadiMaalik: "गाडी मालिक",
      driverBhai: "ड्राइव्हर भाऊ",
      dashboard: "डॅशबोर्ड",
      fleet: "फ्लीट व्यवस्थापन",
      analytics: "टेलीमेट्री लॉग",
      routes: "मार्ग विश्लेषण",
      alerts: "गंभीर सूचना",
      settings: "सिस्टम सेटिंग्ज"
    },
    bn: { 
      selectLang: "ভাষা নির্বাচন করুন", 
      saveDefault: "চালিয়ে যান", 
      name: "বাংলা",
      headOffice: "হেড অফিস",
      kisanView: "কিসান ভিউ",
      gadiMaalik: "গাড়ি মালিক",
      driverBhai: "ড্রাইভার ভাই",
      dashboard: "ড্যাশবোর্ড",
      fleet: "ফ্লিট ম্যানেজমেন্ট",
      analytics: "টেলিমেট্রি লগ",
      routes: "রুট বিশ্লেষণ",
      alerts: "সতর্কতা",
      settings: "সিস্টেম সেটিংস"
    },
    ta: { 
      selectLang: "மொழியைத் தேர்ந்தெடுக்கவும்", 
      saveDefault: "தொடருங்கள்", 
      name: "தமிழ்",
      headOffice: "தலைமை அலுவலகம்",
      kisanView: "விவசாயী பார்வை",
      gadiMaalik: "கார் உரிமையாளர்",
      driverBhai: "ஓட்டுனர் சகோதரன்",
      dashboard: "டாஷ்போர்டு",
      fleet: "கப்பற்படை",
      analytics: "தொலிமாপன பதிவுகள்",
      routes: "வழி பகுப்பாய்வு",
      alerts: "எச்சரிக்கைகள்",
      settings: "அமைப்புகள்"
    },
    te: { 
      selectLang: "భాషను ఎంచుకోండి", 
      saveDefault: "కొనసాగించండి", 
      name: "తెలుగు",
      headOffice: "ప్రధान కార్యాలయం",
      kisanView: "రైతు దృశ్యం",
      gadiMaalik: "గాడీ యజమాని",
      driverBhai: "డ్రైవర్ సోదరుడు",
      dashboard: "డాష్‌బోర్డ్",
      fleet: "ఫ్లీట్ నిర్వహణ",
      analytics: "టెలిమెట్రీ లాగ్‌లు",
      routes: "రూట్ విశ్లేషణ",
      alerts: "హెచ్చరికలు",
      settings: "అమరికలు"
    }
  },

  // --- MENU ---
  menu: {
    en: { dash: "Dashboard", fleet: "Fleet Management", analytics: "Telemetry Logs", routes: "Route Analysis", alerts: "Critical Alerts", settings: "System Settings", myCrops: "My Crops", shipments: "Shipments", navigation: "Navigation", documents: "Documents", liveMap: "Live Map" },
    hi: { dash: "डैशबोर्ड", fleet: "वाहन प्रबंधन", analytics: "टेलीमेट्री लॉग", routes: "मार्ग विश्लेषण", alerts: "गंभीर अलर्ट", settings: "सिस्टम सेटिंग्स", myCrops: "My Crops", shipments: "Shipments", navigation: "Navigation", documents: "Documents", liveMap: "Live Map" },
    gu: { dash: "ડેશબોર્ડ", fleet: "કાફલો સંચાલન", analytics: "ટેલિમેટ્રી લોગ્સ", routes: "રૂટ વિશ્લેષણ", alerts: "ગંભીર ચેતવણીઓ", settings: "સિસ્ટમ સેટિંગ્સ", myCrops: "My Crops", shipments: "Shipments", navigation: "Navigation", documents: "Documents", liveMap: "Live Map" },
    gj: { dash: "ડેશબોર્ડ", fleet: "કાફલો સંચાલન", analytics: "ટેલિમેટ્રી લોગ્સ", routes: "રૂટ વિશ્લેષણ", alerts: "ગંભીર ચેતવણીઓ", settings: "સિસ્ટમ સેટિંગ્સ", myCrops: "My Crops", shipments: "Shipments", navigation: "Navigation", documents: "Documents", liveMap: "Live Map" },
    pa: { dash: "ਡੈਸ਼ਬੋਰਡ", fleet: "ਫਲੀਟ ਪ੍ਰबંधਨ", analytics: "ਟੈਲੀਮੈਟਰੀ ਲੌਗਸ", routes: "ਰੂਟ ਵਿਸ਼ਲਲੇਸ਼ਣ", alerts: "ਨਾਜ਼ੁਕ ਚेतਾਵਨੀਆਂ", settings: "ਸਿਸ्ਟਮ ਸੈਟਿੰਗਾਂ", myCrops: "My Crops", shipments: "Shipments", navigation: "Navigation", documents: "Documents", liveMap: "Live Map" },
    mr: { dash: "डॅशबोर्ड", fleet: "फ्लीट व्यवस्थापन", analytics: "टेलीमेट्री लॉग", routes: "मार्ग विश्लेषण", alerts: "गंभीर सूचना", settings: "सिस्टम सेटिंग्ज", myCrops: "My Crops", shipments: "Shipments", navigation: "Navigation", documents: "Documents", liveMap: "Live Map" },
    bn: { dash: "ড্যাশবোর्ড", fleet: "ফ্লিট ম্যানেজমেন্ট", analytics: "টেলিমেট्री লগ", routes: "রুট বিশ্লেষণ", alerts: "সতর্কতা", settings: "সিস्टेম সেটिংস", myCrops: "My Crops", shipments: "Shipments", navigation: "Navigation", documents: "Documents", liveMap: "Live Map" },
    ta: { dash: "டாஷ్போர्டु", fleet: "கப्பற्படை", analytics: "பதिவुकल", routes: "வழி பกுप्पายvu", alerts: "எച्சरிक्केलिकल", settings: "अमैप्पుकल", myCrops: "My Crops", shipments: "Shipments", navigation: "Navigation", documents: "Documents", liveMap: "Live Map" },
    te: { dash: "డాష్‌బోర్డ్", fleet: "ఫ్లీట్ నిర్వहण", analytics: "టेలिमेট్రీ లाग్‌లು", routes: "రూట్ విশ్లेషణ", alerts: "హెച्चरिकulu", settings: "అমरिkulù", myCrops: "My Crops", shipments: "Shipments", navigation: "Navigation", documents: "Documents", liveMap: "Live Map" }
  },

  // --- DASHBOARD HEALTH (All Languages - All Parameters) ---
  dashboard: {
    en: { 
      healthIndex: "Cargo Health Index", fresh: "Fresh / Safe", risk: "Spoilage Risk!", offline: "Connection Lost", 
      aiMonitor: "AI Monitor (SVM):", safe: "✅ Safe Transit", riskDetected: "⚠️ Risk Detected", waiting: "Waiting for connection...", 
      tempLabel: "Temperature (SHT30)", shockLabel: "Shock (Accel)", rotationLabel: "Rotation Rate", humidityLabel: "Humidity", 
      syncStatus: "Sync Status:", connected: "Connected", disconnected: "Disconnected",
      systemHealth: "System Health", analysis: "Real-time analysis", header: "Fleet Status",
      criticalAlerts: "Critical Alerts (Driver)", noAlerts: "No fleet alerts right now.", items: "items",
      tempSpiked: "High Temperature", tempHigh: "Spiked to 31°C near Nashik", 
      humidityLow: "Low Humidity", humidityDropped: "Dropped below 40% during loading", 
      speedBump: "Speed Bump Hit", shockDetected: "Hard shock detected, check cargo straps",
      signalDrop: "Signal Drop", signalOutage: "3 min outage near Bhiwandi bypass", 
      fleetAlert: "Fleet alert", warning: "Warning", critical: "Critical", resolved: "Resolved", 
      moving: "Moving", active: "Active", signalLost: "Signal Lost", online: "Online"
    },
    hi: { 
      healthIndex: "कार्गो स्वास्थ्य सूचकांक", fresh: "ताजा / सुरक्षित", risk: "खराब होने का जोखिम!", offline: "कनेक्शन खो गया", 
      aiMonitor: "AI मॉनिटर (SVM):", safe: "✅ सुरक्षित पारगमन", riskDetected: "⚠️ जोखिम का पता चला", waiting: "कनेक्शन की प्रतीक्षा...", 
      tempLabel: "तापमान (SHT30)", shockLabel: "झटका (Accel)", rotationLabel: "रोटेशन दर", humidityLabel: "नमी", 
      syncStatus: "सिंक स्थिति:", connected: "जुड़ा हुआ", disconnected: "डिस्कनेक्ट किया गया",
      systemHealth: "सिस्टम स्वास्थ्य", analysis: "रीयल-टाइम विश्लेषण", header: "वाहन स्थिति",
      criticalAlerts: "गंभीर चेतावनी", noAlerts: "अभी कोई फ्लीट चेतावनी नहीं।", items: "आइटम",
      tempSpiked: "उच्च तापमान", tempHigh: "नाशिक के पास 31°C तक बढ़ा", 
      humidityLow: "कम नमी", humidityDropped: "लोडिंग के दौरान 40% से नीचे गिरा", 
      speedBump: "स्पीड बम्प हिट", shockDetected: "कठोर झटका महसूस किया गया, कार्गो स्ट्रैप्स की जांच करें",
      signalDrop: "सिग्नल ड्रॉप", signalOutage: "भिवंडी बाईपास के पास 3 मिनट का आउटेज", 
      fleetAlert: "फ्लीट चेतावनी", warning: "चेतावनी", critical: "गंभीर", resolved: "समाधान किया गया", 
      moving: "चल रहा है", active: "सक्रिय", signalLost: "सिग्नल खो गया", online: "ऑनलाइन"
    },
    gu: { 
      healthIndex: "કાર્ગો હેલ્થ ઇન્ડેક્સ", fresh: "તાજું / સુરક્ષિત", risk: "બગાડનું જોખમ!", offline: "કનેક્શન ખોવાયું", 
      aiMonitor: "AI મોનિટર (SVM):", safe: "✅ સુરક્ષિત પરિવહન", riskDetected: "⚠️ જોખમ મળ્યું", waiting: "કનેક્શનનો ઇંતજાર...", 
      tempLabel: "તાપમાન (SHT30)", shockLabel: "આઘાત (Accel)", rotationLabel: "રોટેશન દર", humidityLabel: "નમી", 
      syncStatus: "સિંક સ્થિતિ:", connected: "જોડાયેલ", disconnected: "જોડાણ તોડવામાં આવ્યું",
      systemHealth: "સિસ્ટમ સ્વાસ્થ્ય", analysis: "રીયલ-ટાઈમ વિશ્લેષણ", header: "ફ્લીટ સ્થિતિ",
      criticalAlerts: "જટિલ સતર્કતાઓ", noAlerts: "આ સમયે કોઈ ફ્લીટ સતર્કતા નથી।", items: "આઇટમ્સ",
      tempSpiked: "ઉચ્ચ તાપમાન", tempHigh: "નાશિક પાસે 31°C સુધી વધ્યું", 
      humidityLow: "નીચી નમી", humidityDropped: "લોડિંગ દરમિયાન 40% ની નીચે ઘટ્યું", 
      speedBump: "સ્પીડ બમ્પ હીટ", shockDetected: "કડક આઘાત મળ્યો, કાર્ગો સ્ટ્રેપ્સ તપાસો",
      signalDrop: "સિગ્નલ ડ્રૉપ", signalOutage: "ભીવંડી બાયપાસ પાસે આઉટેજ", 
      fleetAlert: "ફ્લીટ સતર્કતા", warning: "ચેતવણી", critical: "જટિલ", resolved: "ઉકેલ થયું", 
      moving: "ચાલી રહ્યું છે", active: "સક્રિય", signalLost: "સિગ્નલ ખોવાયું", online: "ઓનલાઈન"
    },
    gj: { 
      healthIndex: "કાર્ગો હેલ્થ ઇન્ડેક્સ", fresh: "તાજું / સુરક્ષિત", risk: "બગાડનું જોખમ!", offline: "કનેક્શન ખોવાયું", 
      aiMonitor: "AI મોનિટર (SVM):", safe: "✅ સુરક્ષિત પરિવહન", riskDetected: "⚠️ જોખમ મળ્યું", waiting: "કનેક્શનનો ઇંતજાર...", 
      tempLabel: "તાપમાન (SHT30)", shockLabel: "આઘાત (Accel)", rotationLabel: "રોટેશન દર", humidityLabel: "નમી", 
      syncStatus: "સિંક સ્થિતિ:", connected: "જોડાયેલ", disconnected: "જોડાણ તોડવામાં આવ્યું",
      systemHealth: "સિસ્ટમ સ્વાસ્થ્ય", analysis: "રીયલ-ટાઈમ વિશ્લેષણ", header: "ફ્લીટ સ્થિતિ",
      criticalAlerts: "જટિલ સતર્કતાઓ", noAlerts: "આ સમયે કોઈ ફ્લીટ સતર્કતા નથી।", items: "આઇટમ્સ",
      tempSpiked: "ઉચ્ચ તાપમાન", tempHigh: "નાશિક પાસે 31°C સુધી વધ્યું", 
      humidityLow: "નીચી નમી", humidityDropped: "લોડિંગ દરમિયાન 40% ની નીચે ઘટ્યું", 
      speedBump: "સ્પીડ બમ્પ હીટ", shockDetected: "કડક આઘાત મળ્યો, કાર્ગો સ્ટ્રેપ્સ તપાસો",
      signalDrop: "સિગ્નલ ડ્રૉપ", signalOutage: "ભીવંડી બાયપાસ પાસે આઉટેજ", 
      fleetAlert: "ફ્લીટ સતર્કતા", warning: "ચેતવણી", critical: "જટિલ", resolved: "ઉકેલ થયું", 
      moving: "ચાલી રહ્યું છે", active: "સક્રિય", signalLost: "સિગ્નલ ખોવાયું", online: "ઓનલાઈન"
    },
    pa: { 
      healthIndex: "ਕਾਰਗੋ ਸਿਹਤ ਸੂਚਕਾਂਕ", fresh: "ਤਾਜਾ / ਸੁਰੱਖਿਅਤ", risk: "ਖਰਾਬੀ ਦਾ ਖਤਰਾ!", offline: "ਕਨੈਕਸ਼ਨ ਖੋ ਗਿਆ", 
      aiMonitor: "AI ਮਨੀਟਰ (SVM):", safe: "✅ ਸੁਰੱਖਿਅਤ ਟਰਾਂਸਿਟ", riskDetected: "⚠️ ਖਤਰਾ ਖੋਜਿਆ", waiting: "ਕਨੈਕਸ਼ਨ ਦਾ ਇੰਤਜ਼ਾਰ...", 
      tempLabel: "ਤਾਪਮਾਨ (SHT30)", shockLabel: "ਸਦਮਾ (Accel)", rotationLabel: "ਰੋਟੇਸ਼ਨ ਦਰ", humidityLabel: "ਨਮੀ", 
      syncStatus: "ਸਿੰਕ ਸਥਿਤੀ:", connected: "ਜੁੜਿਆ ਹੋਇਆ", disconnected: "ਡਿਸਕਨੈਕਟ ਕੀਤਾ ਗਿਆ",
      systemHealth: "ਸਿਸਟਮ ਹੈਲਥ", analysis: "ਰੀਅਲ-ਟਾਇਮ ਵਿਸ਼ਲੇਸ਼ਣ", header: "ਫਲੀਟ ਸਥਿਤੀ",
      criticalAlerts: "ਗੰਭੀਰ ਚੇतાਵਨੀਆਂ", noAlerts: "ਇਸ ਸਮੇਂ ਕੋਈ ਫਲੀਟ ਚੇتાਵਨੀ ਨਹੀਂ।", items: "ਆਈਟਮ",
      tempSpiked: "ਉੱਚ ਤਾਪਮਾਨ", tempHigh: "ਨਾਸ਼ਿਕ ਦੇ ਪਾਸ 31°C ਤੱਕ ਵਧਿਆ", 
      humidityLow: "ਘੱਟ ਨਮੀ", humidityDropped: "ਲੋਡਿੰਗ ਦੌਰਾਨ 40% ਤੋਂ ਥੱਲੇ", 
      speedBump: "ਸਪੀਡ ਬੰਪ ਟੱਕ", shockDetected: "ਸਖ਼ਤ ਸਦਮਾ, ਕਾਰਗੋ ਸਟ੍ਰੈਪ ਚੈੱਕ ਕਰੋ",
      signalDrop: "ਸਿਗਨਲ ਡ੍ਰੌਪ", signalOutage: "ਭਿਵੰਡੀ ਬਾਈਪਾਸ ਕੋਲ ਆਉਟੇਜ", 
      fleetAlert: "ਫਲੀਟ ਚੇতাਵਨੀ", warning: "ਚੇતাਵਨੀ", critical: "ਗੰਭੀਰ", resolved: "ਹੱਲ ਕੀਤਾ", 
      moving: "ਚਲ ਰਿਹਾ ਹੈ", active: "ਸਰਗਰਮ", signalLost: "ਸਿਗਨਲ ਗਾਇਬ", online: "ਆਨਲਾਈਨ"
    },
    mr: { 
      healthIndex: "कार्गो आरोग्य निर्देशांक", fresh: "ताज़ा / सुरक्षित", risk: "खराब होने का जोखिम!", offline: "कनेक्शन खोयाला", 
      aiMonitor: "AI मॉनिटर (SVM):", safe: "✅ सुरक्षित ट्रांजिट", riskDetected: "⚠️ जोखिम मिल गया", waiting: "कनेक्शन की प्रतीक्षा...", 
      tempLabel: "तापमान (SHT30)", shockLabel: "झटका (Accel)", rotationLabel: "रोटेशन दर", humidityLabel: "आर्दता", 
      syncStatus: "सिंक स्थिति:", connected: "जुडलेला", disconnected: "डिस्कनेक्ट केला गेला",
      systemHealth: "सिस्टम आरोग्य", analysis: "रीअल-टाइम विश्लेषण", header: "फ्लीट स्थिति",
      criticalAlerts: "गंभीर सूचना", noAlerts: "या क्षणी कोणतीही फ्लीट सूचना नाही।", items: "आयटम",
      tempSpiked: "उच्च तापमान", tempHigh: "नाशिक जवळ 31°C पर्यंत", 
      humidityLow: "कमी आर्दता", humidityDropped: "लोडिंग दरम्यान 40% खाली", 
      speedBump: "स्पीड बम्प हिट", shockDetected: "कठोर झटका, मालवाहतूक पट्ट्या तपासा",
      signalDrop: "सिग्नल ड्रॉप", signalOutage: "भिवंडी बायपास जवळ आउटेज", 
      fleetAlert: "फ्लीट सूचना", warning: "चेतावणी", critical: "गंभीर", resolved: "सुटके", 
      moving: "हलत आहे", active: "सक्रिय", signalLost: "सिग्नल हरवला", online: "ऑनलाइन"
    },
    bn: { 
      healthIndex: "কার्গো স্বাস্थ্য সূচক", fresh: "তাজা / নিরাপদ", risk: "পচন ঝুঁকি!", offline: "সংযোগ হারিয়েছে", 
      aiMonitor: "AI মনিটর (SVM):", safe: "✅ নিরাপদ ট্রানজিট", riskDetected: "⚠️ ঝুঁকি সনাক্ত", waiting: "সংযোগের জন्য অপेक्ষা...", 
      tempLabel: "তাপমাত्রা (SHT30)", shockLabel: "শক (Accel)", rotationLabel: "ঘূর্ণন হার", humidityLabel: "আর्দ्रता", 
      syncStatus: "সিঙ্ক अवस्था:", connected: "সংযुक्त", disconnected: "সংयোग बिচ्छिन्न",
      systemHealth: "सिस्टम स्वास्थ्य", analysis: "রিয়েल-टाইम বिশ्लेषण", header: "फ्लीट अवस्था",
      criticalAlerts: "गुरुत्वपूर्ण सতर्कता", noAlerts: "এখن কোनো फ্लीট সতर্कता নেই।", items: "আیটেম",
      tempSpiked: "उच्च तापमान", tempHigh: "নাশিক এর কাছে 31°C", 
      humidityLow: "कम आर्दता", humidityDropped: "लोडिंग সময় 40% नीचे", 
      speedBump: "स्पीड बाम्प हीट", shockDetected: "कठिन ड्राफ्ट, कार्गो স्ट्र्यप तपास करो",
      signalDrop: "सिगन्यल ড्রॉप", signalOutage: "भिवाण्डी byপাস आउटेज", 
      fleetAlert: "फ्लीট सतร्कता", warning: "সতร্কता", critical: "गुरुত्वपूर्ण", resolved: "সমाधान", 
      moving: "চलছे", active: "सक्रिय", signalLost: "सिग्नल गायб", online: "অনлाइন"
    },
    ta: { 
      healthIndex: "சरक्कு आरोक्यिய குறियीडু", fresh: "puതിய / बाธुकाप्पु", risk: "कॆट्टुповुम् आबथु!", offline: "इणैप्पु इझ्नथु", 
      aiMonitor: "AI मणिट्टर (SVM):", safe: "✅ பाধुکാпპான போक्కुवरथु", riskDetected: "⚠️ आबथु कण्डरियबथु", waiting: "इणैப्पिर्क्कु कावु...", 
      tempLabel: "वेप्पनिलै (SHT30)", shockLabel: "अथिरच्छि (Accel)", rotationLabel: "सुझरच्छि विकिथम्", humidityLabel: "ईरप्पथम्", 
      syncStatus: "ओत्थिसैप्पु निलै:", connected: "इणैक्कपट्टु", disconnected: "इणैप्पु तुण्डिक्कपट्टु",
      systemHealth: "सिस्टम आरोक्यम्", analysis: "वाज़्नैल्", header: "फ्लीट निलै",
      criticalAlerts: "मुक्किमेन हेच्श्क्काय्कल्", noAlerts: "इप्पो नफ्लीट हेच्श्क्काय्कल् इल्लै।", items: "वरिसैकल्",
      tempSpiked: "उचिक वेप्पनिलै", tempHigh: "नाशिक कारुकु 31°C क्कु उयर्न्थु", 
      humidityLow: "कुरैन्थ ईरप्पथम्", humidityDropped: "लोडिङ्क सമய 40% क्कु कीझे विझ्हुन्थु", 
      speedBump: "स्पीड बामप् हीट", shockDetected: "कधु अथिरच्छि, सरक्कु पेल्त् परीक्षा चेयवु",
      signalDrop: "सिगनल् हरास्", signalOutage: "बिवनडि बाय्पास् кारुку आउटेज्", 
      fleetAlert: "फ्लीट हेच्श्क्काय्", warning: "हेच्श्क्काय्", critical: "मुक्किमेन", resolved: "तीर्क्कपट्टु", 
      moving: "नकरुम्", active: "सक्रिय", signalLost: "सिगनल् इझ्नथु", online: "ओनलाय्न्"
    },
    te: { 
      healthIndex: "కార్గো ఆరोग్య సూచిక", fresh: "ताजా / సურక్ష్ితం", risk: "చెడ్డవ్వ ఆపद!", offline: "కనెక్షన్ కోల్పోయింది", 
      aiMonitor: "AI మానిటర్ (SVM):", safe: "✅ సురక్ష్ित రవాణా", riskDetected: "⚠️ ఆపద గుర్తించబడింది", waiting: "కనెక్션న్ కోసం చేవుతున్నారు...", 
      tempLabel: "ఉష్ణోగ్రत (SHT30)", shockLabel: "షాక్ (Accel)", rotationLabel: "భ్రమణ రేటు", humidityLabel: "ఆర్ద్రत", 
      syncStatus: "సింక్ స్థిति:", connected: "సంయోగం చేయబడింది", disconnected: "కనెక్ట్ చేయబడలేదు",
      systemHealth: "సిస్టమ్ ఆరోగ్యం", analysis: "రియల్-టైమ్ విశ్లేషణ", header: "ఫ్లీట్ స్థిति",
      criticalAlerts: "విమర్ష్నీయ హెచ్చరికలు", noAlerts: "ఇప్పుడు ఫ్లీట్ హెచ్చరికలు లేవు।", items: "అంశాలు",
      tempSpiked: "అధిక ఉష్ణోగ్రత", tempHigh: "నాశిక్ పక్కన 31°C", 
      humidityLow: "తక్కువ ఆర్ద్రత", humidityDropped: "లోడింగ్ సమయంలో 40% కంటే", 
      speedBump: "స్పీడ్ బంప్ హిట్", shockDetected: "కఠిన షాక్, కార్గో పట్టీలను తనిఖీ చేయండి",
      signalDrop: "సిగ్నల్ డ్రాप్", signalOutage: "భీవండీ బైపాస్ వద్ద అవుట్‌జ్", 
      fleetAlert: "ఫ్లీట్ హెచ్చరిక", warning: "హెచ్చరిక", critical: "విమర్ష్నీయ", resolved: "పరిష్కరించబడిన", 
      moving: "కదులుతూ", active: "చేతన", signalLost: "సిగ్నల్ కోల్పోయింది", online: "ఆన్‌లైన్"
    }
  },

  // --- ROUTE ANALYSIS ---
  routes: {
    en: { 
      title: "Route Analysis", 
      sub: "Analyze optimal routes for your deliveries", 
      start: "Start Location", 
      dest: "Destination", 
      analyzeBtn: "Analyze Route", 
      analyzing: "Analyzing...", 
      report: "Analysis Report", 
      time: "Est. Time", 
      dist: "Distance", 
      fuel: "Fuel Cost", 
      risk: "Risk Score", 
      safe: "✅ Recommended Route", 
      poor: "⚠️ High Risk - Consider Alternative" 
    },
    hi: { 
      title: "मार्ग विश्लेषण", 
      sub: "अपनी डिलीवरी के लिए सर्वोत्तम मार्गों का विश्लेषण करें", 
      start: "प्रारंभ स्थान", 
      dest: "गंतव्य", 
      analyzeBtn: "मार्ग विश्लेषण", 
      analyzing: "विश्लेषण हो रहा है...", 
      report: "विश्लेषण रिपोर्ट", 
      time: "अनुमानित समय", 
      dist: "दूरी", 
      fuel: "ईंधन लागत", 
      risk: "जोखिम स्कोर", 
      safe: "✅ अनुशंसित मार्ग", 
      poor: "⚠️ उच्च जोखिम - विकल्प पर विचार करें" 
    },
    gu: { 
      title: "રૂટ વિશ્લેષણ", 
      sub: "તમારી ડિલિવરી માટે શ્રેષ્ઠ માર્ગોનું વિશ્લેષણ કરો", 
      start: "પ્રારંભ સ્થાન", 
      dest: "ગંતવ્ય", 
      analyzeBtn: "માર્ગ વિશ્લેષણ", 
      analyzing: "વિશ્લેષણ થઈ રહ્યું છે...", 
      report: "વિશ્લેષણ રિપોર્ટ", 
      time: "અંદાજિત સમય", 
      dist: "અંતર", 
      fuel: "ઇંધણ ખર્ચ", 
      risk: "જોખમ સ્કોર", 
      safe: "✅ ભલામણ કરેલ માર્ગ", 
      poor: "⚠️ ઉચ્ચ જોખમ - વૈકલ્પિક પર વિચાર કરો" 
    },
    gj: { 
      title: "રૂટ વિશ્લેષણ", 
      sub: "તમારી ડિલિવરી માટે શ્રેષ્ઠ માર્ગોનું વિશ્લેષણ કરો", 
      start: "પ્રારંભ સ્થાન", 
      dest: "ગંતવ્ય", 
      analyzeBtn: "માર્ગ વિશ્લેષણ", 
      analyzing: "વિશ્લેષણ થઈ રહ્યું છે...", 
      report: "વિશ્લેષણ રિપોર્ટ", 
      time: "અંદાજિત સમય", 
      dist: "અંતર", 
      fuel: "ઇંધણ ખર્ચ", 
      risk: "જોખમ સ્કોર", 
      safe: "✅ ભલામણ કરેલ માર્ગ", 
      poor: "⚠️ ઉચ્ચ જોખમ - વૈકલ્પિક પર વિચાર કરો" 
    },
    pa: { 
      title: "ਰੂਟ ਵਿਸ਼ਲੇਸ਼ਣ", 
      sub: "ਆਪਣੀ ਡਿਲੀਵਰੀ ਲਈ ਸਭ ਤੋਂ ਵਧੀਆ ਰੂਟਾਂ ਦਾ ਵਿਸ਼ਲੇਸ਼ਣ ਕਰੋ", 
      start: "ਸ਼ੁਰੂਆਤੀ ਸਥਾਨ", 
      dest: "ਮੰਜ਼ਿਲ", 
      analyzeBtn: "ਰੂਟ ਵਿਸ਼ਲੇਸ਼ਣ", 
      analyzing: "ਵਿਸ਼ਲੇਸ਼ਣ ਹੋ ਰਿਹਾ ਹੈ...", 
      report: "ਵਿਸ਼ਲੇਸ਼ਣ ਰਿਪੋਰਟ", 
      time: "ਅੰਦਾਜ਼ਨ ਸਮਾਂ", 
      dist: "ਦੂਰੀ", 
      fuel: "ਈਂਧਨ ਲਾਗਤ", 
      risk: "ਜੋਖਮ ਸਕੋਰ", 
      safe: "✅ ਸਿਫਾਰਸ਼ ਕੀਤਾ ਰੂਟ", 
      poor: "⚠️ ਉੱਚ ਜੋਖਮ - ਵਿਕਲਪਿਕ ਬਾਰੇ ਸੋਚੋ" 
    },
    mr: { 
      title: "मार्ग विश्लेषण", 
      sub: "तुमच्या डिलिव्हरीसाठी सर्वोत्तम मार्गांचे विश्लेषण करा", 
      start: "प्रारंभ स्थान", 
      dest: "गंतव्य", 
      analyzeBtn: "मार्ग विश्लेषण", 
      analyzing: "विश्लेषण होत आहे...", 
      report: "विश्लेषण अहवाल", 
      time: "अंदाजे वेळ", 
      dist: "अंतर", 
      fuel: "इंधन खर्च", 
      risk: "जोखीम स्कोअर", 
      safe: "✅ शिफारस केलेला मार्ग", 
      poor: "⚠️ उच्च जोखीम - पर्यायी विचारात घ्या" 
    },
    bn: { 
      title: "রুট বিশ্লেষণ", 
      sub: "আপনার ডেলিভারির জন্য সর্বোত্তম রুট বিশ্লেষণ করুন", 
      start: "শুরু অবস্থান", 
      dest: "গন্তব্য", 
      analyzeBtn: "রুট বিশ্লেষণ", 
      analyzing: "বিশ্লেষণ হচ্ছে...", 
      report: "বিশ্লেষণ রিপোর্ট", 
      time: "আনুমানিক সময়", 
      dist: "দূরত্ব", 
      fuel: "জ্বালানি খরচ", 
      risk: "ঝুঁকি স্কোর", 
      safe: "✅ সুপারিশকৃত রুট", 
      poor: "⚠️ উচ্চ ঝুঁকি - বিকল্প বিবেচনা করুন" 
    },
    ta: { 
      title: "வழி பகுப்பாய்வு", 
      sub: "உங்கள் விநியோகத்திற்கான சிறந்த வழிகளை பகுப்பாய்வு செய்யுங்கள்", 
      start: "தொடக்க இடம்", 
      dest: "இலக்கு", 
      analyzeBtn: "வழி பகுப்பாய்வு", 
      analyzing: "பகுப்பாய்வு செய்யப்படுகிறது...", 
      report: "பகுப்பாய்வு அறிக்கை", 
      time: "மதிப்பிட்ட நேரம்", 
      dist: "தூரம்", 
      fuel: "எரிபொருள் செலவு", 
      risk: "ஆபத்து மதிப்பெண்", 
      safe: "✅ பரிந்துரைக்கப்பட்ட வழி", 
      poor: "⚠️ உயர் ஆபத்து - மாற்றை கருத்தில் கொள்ளுங்கள்" 
    },
    te: { 
      title: "రూట్ విశ్లేషణ", 
      sub: "మీ డెలివరీకి ఉత్తమ మార్గాలను విశ్లేషించండి", 
      start: "ప్రారంభ స్థానం", 
      dest: "గమ్యం", 
      analyzeBtn: "రూట్ విశ్లేషణ", 
      analyzing: "విశ్లేషణ జరుగుతోంది...", 
      report: "విశ్లేషణ నివేదిక", 
      time: "అంచనా సమయం", 
      dist: "దూరం", 
      fuel: "ఇంధన వ్యయం", 
      risk: "ప్రమాద స్కోర్", 
      safe: "✅ సిఫార్సు చేయబడిన రూట్", 
      poor: "⚠️ అధిక ప్రమాదం - ప్రత్యామ్నాయాన్ని పరిగణించండి" 
    }
  },

  // --- ANALYTICS (TELEMETRY LOGS) ---
  analytics: {
    en: { 
      title: "Telemetry Analytics", 
      total: "Total Shipments", 
      tempTrend: "Temperature Trends (6 Months)", 
      topRoutes: "Top Routes Performance", 
      efficiency: "Efficiency", 
      exportPDF: "Export PDF Report", 
      shipments: "Shipments", 
      avgTime: "Avg Delivery Time", 
      riskFactor: "Risk Factor", 
      revenue: "Revenue Protected" 
    },
    hi: { 
      title: "टेलीमेट्री एनालिटिक्स", 
      total: "कुल शिपमेंट", 
      tempTrend: "तापमान रुझान (6 महीने)", 
      topRoutes: "शीर्ष मार्ग प्रदर्शन", 
      efficiency: "दक्षता", 
      exportPDF: "PDF रिपोर्ट निर्यात करें", 
      shipments: "शिपमेंट", 
      avgTime: "औसत डिलीवरी समय", 
      riskFactor: "जोखिम कारक", 
      revenue: "राजस्व संरक्षित" 
    },
    gu: { 
      title: "ટેલિમેટ્રી એનાલિટિક્સ", 
      total: "કુલ શિપમેન્ટ", 
      tempTrend: "તાપમાન વલણો (6 મહિના)", 
      topRoutes: "ટોચના માર્ગ પ્રદર્શન", 
      efficiency: "કાર્યક્ષમતા", 
      exportPDF: "PDF રિપોર્ટ નિકાસ કરો", 
      shipments: "શિપમેન્ટ", 
      avgTime: "સરેરાશ ડિલિવરી સમય", 
      riskFactor: "જોખમ પરિબળ", 
      revenue: "આવક સુરક્ષિત" 
    },
    gj: { 
      title: "ટેલિમેટ્રી એનાલિટિક્સ", 
      total: "કુલ શિપમેન્ટ", 
      tempTrend: "તાપમાન વલણો (6 મહિના)", 
      topRoutes: "ટોચના માર્ગ પ્રદર્શન", 
      efficiency: "કાર્યક્ષમતા", 
      exportPDF: "PDF રિપોર્ટ નિકાસ કરો", 
      shipments: "શિપમેન્ટ", 
      avgTime: "સરેરાશ ડિલિવરી સમય", 
      riskFactor: "જોખમ પરિબળ", 
      revenue: "આવક સુરક્ષિત" 
    },
    pa: { 
      title: "ਟੈਲੀਮੈਟਰੀ ਵਿਸ਼ਲੇਸ਼ਣ", 
      total: "ਕੁੱਲ ਸ਼ਿਪਮੈਂਟ", 
      tempTrend: "ਤਾਪਮਾਨ ਰੁਝਾਨ (6 ਮਹੀਨੇ)", 
      topRoutes: "ਸਿਖਰ ਰੂਟ ਪ੍ਰਦਰਸ਼ਨ", 
      efficiency: "ਕੁਸ਼ਲਤਾ", 
      exportPDF: "PDF ਰਿਪੋਰਟ ਨਿਰਯਾਤ ਕਰੋ", 
      shipments: "ਸ਼ਿਪਮੈਂਟ", 
      avgTime: "ਔਸਤ ਡਿਲੀਵਰੀ ਸਮਾਂ", 
      riskFactor: "ਜੋਖਮ ਕਾਰਕ", 
      revenue: "ਆਮਦਨ ਸੁਰੱਖਿਅਤ" 
    },
    mr: { 
      title: "टेलीमेट्री विश्लेषण", 
      total: "एकूण शिपमेंट", 
      tempTrend: "तापमान ट्रेंड (6 महिने)", 
      topRoutes: "शीर्ष मार्ग कामगिरी", 
      efficiency: "कार्यक्षमता", 
      exportPDF: "PDF अहवाल निर्यात करा", 
      shipments: "शिपमेंट", 
      avgTime: "सरासरी डिलिव्हरी वेळ", 
      riskFactor: "जोखीम घटक", 
      revenue: "महसूल संरक्षित" 
    },
    bn: { 
      title: "টেলিমেট্রি বিশ্লেষণ", 
      total: "মোট শিপমেন্ট", 
      tempTrend: "তাপমাত্রা প্রবণতা (৬ মাস)", 
      topRoutes: "শীর্ষ রুট পারফরম্যান্স", 
      efficiency: "দক্ষতা", 
      exportPDF: "PDF রিপোর্ট রপ্তানি করুন", 
      shipments: "শিপমেন্ট", 
      avgTime: "গড় ডেলিভারি সময়", 
      riskFactor: "ঝুঁকির কারণ", 
      revenue: "রাজস্ব সুরক্ষিত" 
    },
    ta: { 
      title: "டெலிமெட்ரி பகுப்பாய்வு", 
      total: "மொத்த ஏற்றுமதிகள்", 
      tempTrend: "வெப்பநிலை போக்குகள் (6 மாதங்கள்)", 
      topRoutes: "சிறந்த வழி செயல்திறன்", 
      efficiency: "திறன்", 
      exportPDF: "PDF அறிக்கையை ஏற்றுமதி செய்யுங்கள்", 
      shipments: "ஏற்றுமதிகள்", 
      avgTime: "சராசரி விநியோக நேரம்", 
      riskFactor: "ஆபத்து காரணி", 
      revenue: "வருவாய் பாதுகாக்கப்பட்டது" 
    },
    te: { 
      title: "టెలిమెట్రి విశ్లేషణ", 
      total: "మొత్తం షిప్‌మెంట్లు", 
      tempTrend: "ఉష్ణోగ్రత ట్రెండ్‌లు (6 నెలలు)", 
      topRoutes: "అగ్ర మార్గాల పనితీరు", 
      efficiency: "సామర్థ్యం", 
      exportPDF: "PDF నివేదికను ఎగుమతి చేయండి", 
      shipments: "షిప్‌మెంట్లు", 
      avgTime: "సగటు డెలివరీ సమయం", 
      riskFactor: "ప్రమాద కారకం", 
      revenue: "రాబడి రక్షించబడింది" 
    }
  },
  // --- ALERTS PAGE ---
  alerts: {
    en: { 
      title: "Critical Alerts", 
      sub: "Real-time monitoring of all fleet & sensor alerts", 
      incidents: "Total Incidents", 
      active: "Active Issues", 
      loss: "Est. Loss", 
      export: "Export Report",
      allBtn: "All",
      criticalBtn: "Critical",
      resolvedBtn: "Resolved",
      tableType: "Type",
      tableTruck: "Truck",
      tableValue: "Value",
      tableLocation: "Location",
      tableTime: "Time",
      tableStatus: "Status"
    },
    hi: { 
      title: "गंभीर अलर्ट", 
      sub: "सभी फ्लीट और सेंसर अलर्ट की रीयल-टाइम निगरानी", 
      incidents: "कुल घटनाएं", 
      active: "सक्रिय समस्याएं", 
      loss: "अनुमानित हानि", 
      export: "रिपोर्ट निर्यात करें",
      allBtn: "सभी",
      criticalBtn: "गंभीर",
      resolvedBtn: "समाधान",
      tableType: "प्रकार",
      tableTruck: "ट्रक",
      tableValue: "मान",
      tableLocation: "स्थान",
      tableTime: "समय",
      tableStatus: "स्थिति"
    },
    gu: { 
      title: "ગંભીર ચેતવણીઓ", 
      sub: "તમામ ફ્લીટ અને સેન્સર ચેતવણીઓની વાસ્તવિક-સમય નીરીક્ષણ", 
      incidents: "કુલ ઘટનાઓ", 
      active: "સક્રિય સમસ્યાઓ", 
      loss: "અંદાજિત નુકસાન", 
      export: "રિપોર્ટ નિકાસ કરો",
      allBtn: "બધું",
      criticalBtn: "ગંભીર",
      resolvedBtn: "ઉકેલી",
      tableType: "પ્રકાર",
      tableTruck: "ટ્રક",
      tableValue: "મૂલ્ય",
      tableLocation: "સ્થાન",
      tableTime: "સમય",
      tableStatus: "સ્થિતિ"
    },
    gj: { 
      title: "ગંભીર ચેતવણીઓ", 
      sub: "તમામ ફ્લીટ અને સેન્સર ચેતવણીઓની વાસ્તવિક-સમય નીરીક્ષણ", 
      incidents: "કુલ ઘટનાઓ", 
      active: "સક્રિય સમસ્યાઓ", 
      loss: "અંદાજિત નુકસાન", 
      export: "રિપોર્ટ નિકાસ કરો",
      allBtn: "બધું",
      criticalBtn: "ગંભીર",
      resolvedBtn: "ઉકેલી",
      tableType: "પ્રકાર",
      tableTruck: "ટ્રક",
      tableValue: "મૂલ્ય",
      tableLocation: "સ્થાન",
      tableTime: "સમય",
      tableStatus: "સ્થિતિ"
    },
    pa: { 
      title: "ਨਾਜ਼ੁਕ ਚੇतાਵਨੀਆਂ", 
      sub: "ਸਭ ਫ਼ਲੀਟ ਅਤੇ ਸੈਂਸਰ ਅਲਰਟਾਂ ਦੀ ਰੀਅਲ-ਟਾਈਮ ਨਿਗਰਾਨੀ", 
      incidents: "ਕੁੱਲ ਘਟਨਾਵਾਂ", 
      active: "ਸਰਗਰਮ ਮੁੱਦੇ", 
      loss: "ਅਨੁਮਾਨਿਤ ਨੁਕਸਾਨ", 
      export: "ਰਿਪੋਰਟ ਬਰਾਮਦ ਕਰੋ",
      allBtn: "ਸਭ",
      criticalBtn: "ਨਾਜ਼ੁਕ",
      resolvedBtn: "ਹੱਲ",
      tableType: "ਕਿਸਮ",
      tableTruck: "ਟ੍ਰਕ",
      tableValue: "ਮੁੱਲ",
      tableLocation: "ਸਥਾਨ",
      tableTime: "ਸਮਾਂ",
      tableStatus: "ਸਥਿਤੀ"
    },
    mr: { 
      title: "गंभीर सूचनाएं", 
      sub: "सर्व फ्लीट आणि सेन्सर अलर्टची रीयल-टाइम निरीक्षण", 
      incidents: "एकूण घटना", 
      active: "सक्रिय समस्या", 
      loss: "अनुमानित नुकसान", 
      export: "अहवाल निर्यात करा",
      allBtn: "सर्व",
      criticalBtn: "गंभीर",
      resolvedBtn: "सोडविले",
      tableType: "प्रकार",
      tableTruck: "ट्रक",
      tableValue: "मूल्य",
      tableLocation: "स्थान",
      tableTime: "वेळ",
      tableStatus: "स्थिती"
    },
    bn: { 
      title: "গুরুতর সতর্কতা", 
      sub: "সমস্ত ফ্লিট এবং সেন্সর সতর্কতার রিয়েল-টাইম পর্যবেক্ষণ", 
      incidents: "মোট ঘটনা", 
      active: "সক্রিয় সমস্যা", 
      loss: "আনুমানিক ক্ষতি", 
      export: "রিপোর্ট রপ্তানি করুন",
      allBtn: "সমস্ত",
      criticalBtn: "গুরুতর",
      resolvedBtn: "সমাধান",
      tableType: "ধরন",
      tableTruck: "ট্রক",
      tableValue: "মূল্য",
      tableLocation: "অবস্থান",
      tableTime: "সময়",
      tableStatus: "স্থিতি"
    },
    ta: { 
      title: "முக்கியமான எச்சரிக்கைகள்", 
      sub: "அனைத்து பணிக்கப்பு மற்றும் சென்சாரும் உணர்த்தலின் நிஜ-நேர கண்காணிப்பு", 
      incidents: "மொத்த நிகழ்வுகள்", 
      active: "செயல்படும் பிரச्சனைகள்", 
      loss: "மதிப்பிடப்பட்ட இழப்பு", 
      export: "அறிக்கை ஏற்றுமதி",
      allBtn: "அனைத்தும்",
      criticalBtn: "முக்கியமான",
      resolvedBtn: "தீர்க்கப்பட்ட",
      tableType: "வகை",
      tableTruck: "ட்ரக்",
      tableValue: "மதிப்பு",
      tableLocation: "இடம்",
      tableTime: "நேரம்",
      tableStatus: "நிலை"
    },
    te: { 
      title: "క్లిష్ట అలర్ట్‌లు", 
      sub: "అన్ని ఫ్లీట్ మరియు సెన్సర్ సতర్కతల రియల్-టైమ్ పర్యవేక్ష", 
      incidents: "మొత్తం సంఘటనలు", 
      active: "చేతన సమస్యలు", 
      loss: "అంచనా నష్టం", 
      export: "నివేదిక ఎగుమతి",
      allBtn: "అన్నీ",
      criticalBtn: "క్లిష్ట",
      resolvedBtn: "పరిష్కారం",
      tableType: "రకం",
      tableTruck: "ట్రక్",
      tableValue: "విలువ",
      tableLocation: "స్థానం",
      tableTime: "సమయం",
      tableStatus: "స్థితి"
    }
  },
  // --- CITIES TRANSLATION ---
  cities: {
    "Lavad": { en: "Lavad", hi: "लवाड़", gu: "લવાડ", pa: "ਲਵਾਡ", mr: "लवाड", bn: "লভাড", ta: "ளভாड", te: "లవాడ" },
    "Gandhinagar": { en: "Gandhinagar", hi: "गांधीनगर", gu: "ગાંધીનગર", pa: "ਗਾਂਧੀਨਗਰ", mr: "गांधीनगर", bn: "গান্ধীনগর", ta: "கந்திநகர்", te: "గాంధీనగర" },
    "Pune": { en: "Pune", hi: "पुणे", gu: "પુણે", pa: "ਪੁਣੇ", mr: "पुणे", bn: "পুণে", ta: "புணে", te: "పూణె" },
    "Mumbai": { en: "Mumbai", hi: "मुंबई", gu: "મુંબઈ", pa: "ਮੁੰਬਈ", mr: "मुंबई", bn: "মুম্বাই", ta: "மும்பை", te: "ముంబై" },
    "Nashik": { en: "Nashik", hi: "नाशिक", gu: "નાશીક", pa: "ਨਾਸ਼ਿਕ", mr: "नाशिक", bn: "নাশিক", ta: "நாசிக்", te: "నాశిక్" },
    "Surat": { en: "Surat", hi: "सूरत", gu: "સુરત", pa: "ਸੂਰਤ", mr: "सुरत", bn: "সুরাট", ta: "சூரத்", te: "సూరత్" },
    "Vadodara": { en: "Vadodara", hi: "वडोदरा", gu: "વડોદરા", pa: "ਵਡੋਦਰਾ", mr: "वडोदरा", bn: "ভদোদরা", ta: "வதோதरा", te: "వడోదర" },
    "Delhi": { en: "Delhi", hi: "दिल्ली", gu: "દિલ્હી", pa: "ਦਿੱਲੀ", mr: "दिल्ली", bn: "দিল्লি", ta: "டெல्लி", te: "ఢిల्లీ" },
    "Ahmedabad": { en: "Ahmedabad", hi: "अहमदाबाद", gu: "અમદાવાદ", pa: "ਅਹਿਮਦਾਬਾਦ", mr: "अहमदाबाद", bn: "আহমেদाবाद", ta: "அகமதாபாத्", te: "అహ్మదాబాద्" },
    "Bangalore": { en: "Bangalore", hi: "बंगलौर", gu: "બેંગ્લોર", pa: "ਬੈਂਗਲুरూ", mr: "बेंगळुरू", bn: "বেঙ्গালुরु", ta: "பெங்களூरு", te: "బెంగళూరు" },
    "Chennai": { en: "Chennai", hi: "चेन्नई", gu: "ચેન્નાઈ", pa: "ਚੇनई", mr: "चेन्नई", bn: "চেन्नाই", ta: "சென्नை", te: "చెన్నై" },
    "Hyderabad": { en: "Hyderabad", hi: "हैदराबाद", gu: "હૈદરાબાદ", pa: "ਹੈdराবाद", mr: "हैदराबाद", bn: "হায়দরাবাद", ta: "ஹைtराबात्", te: "హైదరాబాద्" }
  },

  // --- TRIP HISTORY & COMPLIANCE REPORTING ---
  tripHistory: {
    en: {
      tripHistory: "Trip History & Compliance",
      loadingTrips: "Loading trip data from SD card...",
      noTrips: "No trips found in selected date range",
      dateFrom: "From Date",
      dateTo: "To Date",
      csvLanguage: "CSV Language",
      generateCSV: "Generate CSV",
      demoCSV: "Demo CSV",
      downloadSuccess: "CSV downloaded successfully!",
      downloadError: "Error downloading CSV",
      statistics: "Statistics",
      totalTrips: "Total Trips",
      totalDistance: "Total Distance (km)",
      totalDuration: "Total Duration (hrs)",
      avgTemp: "Average Temperature",
      maxTemp: "Maximum Temperature",
      minTemp: "Minimum Temperature",
      compliancePassed: "Compliance Passed",
      tripID: "Trip ID",
      startTime: "Start Time",
      endTime: "End Time",
      duration: "Duration (min)",
      distance: "Distance (km)",
      temperature: "Avg Temp (°C)",
      humidity: "Humidity (%)",
      compliance: "Compliance Status",
      passed: "Passed",
      failed: "Failed",
      warning: "Warning",
      sdCardData: "SD Card Data",
      autoExtract: "Auto Extract from ESP32",
      manualRefresh: "Manual Refresh"
    },
    hi: {
      tripHistory: "यात्रा इतिहास और अनुपालन",
      loadingTrips: "एसडी कार्ड से यात्रा डेटा लोड हो रहा है...",
      noTrips: "चयनित दिनांक सीमा में कोई यात्रा नहीं मिली",
      dateFrom: "शुरू दिनांक",
      dateTo: "समाप्त दिनांक",
      csvLanguage: "CSV भाषा",
      generateCSV: "CSV उत्पन्न करें",
      demoCSV: "डेमो CSV",
      downloadSuccess: "CSV सफलतापूर्वक डाउनलोड हुई!",
      downloadError: "CSV डाउनलोड करने में त्रुटि",
      statistics: "आंकड़े",
      totalTrips: "कुल यात्राएं",
      totalDistance: "कुल दूरी (किमी)",
      totalDuration: "कुल अवधि (घंटे)",
      avgTemp: "औसत तापमान",
      maxTemp: "अधिकतम तापमान",
      minTemp: "न्यूनतम तापमान",
      compliancePassed: "अनुपालन पास",
      tripID: "यात्रा ID",
      startTime: "शुरुआत का समय",
      endTime: "समाप्ति का समय",
      duration: "अवधि (मिनट)",
      distance: "दूरी (किमी)",
      temperature: "औसत तापमान (°C)",
      humidity: "आर्द्रता (%)",
      compliance: "अनुपालन स्थिति",
      passed: "पास",
      failed: "विफल",
      warning: "चेतावनी",
      sdCardData: "एसडी कार्ड डेटा",
      autoExtract: "ESP32 से स्वचालित निष्कर्षण",
      manualRefresh: "मैनुअल ताज़ा करें"
    },
    gu: {
      tripHistory: "ટ્રિપ ઇતિહાસ અને સમર્થન",
      loadingTrips: "SD કાર્ડ થી ટ્રિપ ડેટા લોડ થઇ રહ્યો છે...",
      noTrips: "પસંદ કરેલી તારીખ શ્રેણીમાં કોઈ ટ્રિપ નથી",
      dateFrom: "શરૂઆતની તારીખ",
      dateTo: "સમાપ્તીની તારીખ",
      csvLanguage: "CSV ભાષા",
      generateCSV: "CSV જનરેટ કરો",
      demoCSV: "ડેમો CSV",
      downloadSuccess: "CSV સફળતાપૂર્વક ડાউનલોડ થઇ ગયો!",
      downloadError: "CSV ડાউનલોડ કરવામાં ભૂલ",
      statistics: "આંકડાશાસ્ત્ર",
      totalTrips: "કુલ ટ્રિપ્સ",
      totalDistance: "કુલ અંતર (કિમી)",
      totalDuration: "કુલ અવધિ (કલાક)",
      avgTemp: "સરેરાશ તાપમાન",
      maxTemp: "મહત્તમ તાપમાન",
      minTemp: "ન્યૂનતમ તાપમાન",
      compliancePassed: "સમર્થન પાસ",
      tripID: "ટ્રિપ ID",
      startTime: "શરૂઆતનો સમય",
      endTime: "સમાપ્તીનો સમય",
      duration: "અવધિ (મિનિટ)",
      distance: "અંતર (કિમી)",
      temperature: "સરેરાશ તાપમાન (°C)",
      humidity: "આર્દ્રતા (%)",
      compliance: "સમર્થન સ્થિતિ",
      passed: "પાસ",
      failed: "નિષ્ફળ",
      warning: "ચેતવણી",
      sdCardData: "SD કાર્ડ ડેટા",
      autoExtract: "ESP32 થી સ્વચાલિત અર્કન",
      manualRefresh: "મેનુઅલ તાજુ કરો"
    },
    pa: {
      tripHistory: "ਯਾਤਰਾ ਇਤਿਹਾਸ ਅਤੇ ਅਨੁਸਾਰੀ",
      loadingTrips: "SD ਕਾਰਡ ਤੋਂ ਯਾਤਰਾ ਡੇਟਾ ਲੋਡ ਹੋ ਰਿਹਾ ਹੈ...",
      noTrips: "ਚੁਣੀ ਗਈ ਤਾਰੀਖ ਦੀ ਰੇਂਜ ਵਿੱਚ ਕੋਈ ਯਾਤਰਾ ਨਹੀਂ",
      dateFrom: "ਸ਼ੁਰੂਆਤ ਤਾਰੀਖ",
      dateTo: "ਸਮਾਪਤੀ ਤਾਰੀਖ",
      csvLanguage: "CSV ਭਾਸ਼ਾ",
      generateCSV: "CSV ਤਿਆਰ ਕਰੋ",
      demoCSV: "ਡੇਮੋ CSV",
      downloadSuccess: "CSV ਸਫਲਤਾਪੂਰਵਕ ਡਾਊਨਲੋਡ ਹੋ ਗਿਆ!",
      downloadError: "CSV ਡਾਊਨਲੋਡ ਕਰਨ ਵਿੱਚ ਗਲਤੀ",
      statistics: "ਅੰਕੜੇ",
      totalTrips: "ਕੁਲ ਯਾਤਰਾਏਂ",
      totalDistance: "ਕੁਲ ਦੂਰੀ (ਕਿਮੀ)",
      totalDuration: "ਕੁਲ ਮਿਆਦ (ਘੰਟੇ)",
      avgTemp: "ਔਸਤ ਤਾਪਮਾਨ",
      maxTemp: "ਵੱਧ ਤੋਂ ਵੱਧ ਤਾਪਮਾਨ",
      minTemp: "ਘੱਟ ਤੋਂ ਘੱਟ ਤਾਪਮਾਨ",
      compliancePassed: "ਅਨੁਸਾਰੀ ਪਾਸ",
      tripID: "ਯਾਤਰਾ ID",
      startTime: "ਸ਼ੁਰੂਆਤ ਦਾ ਸਮਾਂ",
      endTime: "ਸਮਾਪਤੀ ਦਾ ਸਮਾਂ",
      duration: "ਮਿਆਦ (ਮਿੰਟ)",
      distance: "ਦੂਰੀ (ਕਿਮੀ)",
      temperature: "ਔਸਤ ਤਾਪਮਾਨ (°C)",
      humidity: "ਨਮੀ (%)",
      compliance: "ਅਨੁਸਾਰੀ ਸਥਿਤੀ",
      passed: "ਪਾਸ",
      failed: "ਅਸਫਲ",
      warning: "ਚੇਤਾਵਨੀ",
      sdCardData: "SD ਕਾਰਡ ਡੇਟਾ",
      autoExtract: "ESP32 ਤੋਂ ਸਵੈ-ਨਿਕਾਸ",
      manualRefresh: "ਮੈਨੁਅਲ ਤਾਜ਼ਾ ਕਰੋ"
    },
    mr: {
      tripHistory: "ट्रिप इतिहास आणि अनुपालन",
      loadingTrips: "SD कार्ड मधून ट्रिप डेटा लोड होत आहे...",
      noTrips: "निवडलेल्या तारखेच्या श्रेणीमध्ये कोणत्याही ट्रिप आढळले नाही",
      dateFrom: "सुरुवातीचा दिनांक",
      dateTo: "समाप्तीचा दिनांक",
      csvLanguage: "CSV भाषा",
      generateCSV: "CSV तयार करा",
      demoCSV: "डेमो CSV",
      downloadSuccess: "CSV यशस्वीरित्या डाउनलोड झाली!",
      downloadError: "CSV डाउनलोड करताना त्रुटी",
      statistics: "आंकडे",
      totalTrips: "एकूण ट्रिप्स",
      totalDistance: "एकूण अंतर (किमी)",
      totalDuration: "एकूण कालावधी (तास)",
      avgTemp: "सरासरी तापमान",
      maxTemp: "जास्तीत जास्त तापमान",
      minTemp: "किमानचे तापमान",
      compliancePassed: "अनुपालन पास",
      tripID: "ट्रिप ID",
      startTime: "सुरुवातीची वेळ",
      endTime: "समाप्तीची वेळ",
      duration: "कालावधी (मिनिटे)",
      distance: "अंतर (किमी)",
      temperature: "सरासरी तापमान (°C)",
      humidity: "आर्द्रता (%)",
      compliance: "अनुपालन स्थिती",
      passed: "पास",
      failed: "अपयशी",
      warning: "सावधानी",
      sdCardData: "SD कार्ड डेटा",
      autoExtract: "ESP32 मधून स्वयंचलित अर्क",
      manualRefresh: "मॅनुअल ताजेतर करा"
    },
    bn: {
      tripHistory: "ট্রিপ ইতিহাস এবং সম্মতি",
      loadingTrips: "SD কার্ড থেকে ট্রিপ ডেটা লোড হচ্ছে...",
      noTrips: "নির্বাচিত তারিখের পরিসরে কোনো ট্রিপ পাওয়া যায়নি",
      dateFrom: "শুরুর তারিখ",
      dateTo: "শেষ তারিখ",
      csvLanguage: "CSV ভাষা",
      generateCSV: "CSV তৈরি করুন",
      demoCSV: "ডেমো CSV",
      downloadSuccess: "CSV সফলভাবে ডাউনলোড হয়েছে!",
      downloadError: "CSV ডাউনলোড করতে ত্রুটি",
      statistics: "পরিসংখ্যান",
      totalTrips: "মোট ট্রিপ",
      totalDistance: "মোট দূরত্ব (কিমি)",
      totalDuration: "মোট সময়কাল (ঘন্টা)",
      avgTemp: "গড় তাপমাত্রা",
      maxTemp: "সর্বাধিক তাপমাত্রা",
      minTemp: "সর্বনিম্ন তাপমাত্রা",
      compliancePassed: "সম্মতি পাস",
      tripID: "ট্রিপ আইডি",
      startTime: "শুরুর সময়",
      endTime: "শেষ সময়",
      duration: "সময়কাল (মিনিট)",
      distance: "দূরত্ব (কিমি)",
      temperature: "গড় তাপমাত্রা (°C)",
      humidity: "আর্দ্রতা (%)",
      compliance: "সম্মতি অবস্থা",
      passed: "পাস",
      failed: "ব্যর্থ",
      warning: "সতর্কতা",
      sdCardData: "SD কার্ড ডেটা",
      autoExtract: "ESP32 থেকে স্বয়ংক্রিয় নিষ্কাশন",
      manualRefresh: "ম্যানুয়াল রিফ্রেশ"
    }
  }
};
